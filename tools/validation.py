...
from rodnet.core.post_processing import write_dets_results, write_dets_results_single_frame, write_dets_results_single_frame_submit
...
if __name__ == "__main__":
    ...
    """
    dataset = CRUW(data_root=dataset_configs['data_root'], sensor_config_name='sensor_config_rod2021')
    ...
    truth_dir = dataset_configs['val_root']
    AP, AR = evaluate_rod2021_APAR(submit_dir, truth_dir, dataset)
    epoch_i = int(os.path.split(args.checkpoint)[-1][6:8])
    print('epoch: %d, AP: %.4f, AR: %.4f' % (epoch_i, AP, AR))
