...
def parse_args():
    ...
    parser.add_argument('--overwrite', action="store_true", default=False, help="overwrite prepared data if exist")
    args = parser.parse_args()
    return args


def get_n_frame(txt_path):
    with open(txt_path, 'r') as f:
        data = f.readlines()
    frame_id, r, a, class_name, scores = data[-1].rstrip().split()
...
def load_anno_txt(txt_path, n_frame, dataset):
    ...
    with open(txt_path, 'r') as f:
        data = f.readlines()
    for line in data:
        frame_id, r, a, class_name, scores = line.rstrip().split()
...
def prepare_data(dataset, config_dict, data_dir, split, save_dir, viz=False, overwrite=False):
    ...
    set_cfg['subdir'] = ''  # added
    radar_root = config_dict['dataset_cfg']['radar_root']
    ...
    if overwrite:
        print("overwrite, delete existed data.")
    ...
    for seq in sets_seqs:
        ...
        radar_seq_path = os.path.join(radar_root, set_cfg['subdir'], seq)
        ...
        # n_frame = get_n_frame(seq_anno_path)
        radar_dir = os.path.join(radar_seq_path, dataset.sensor_cfg.radar_cfg['chirp_folder'])
