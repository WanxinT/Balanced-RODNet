...
from rodnet.datasets.CRDataset_test_online import CRDataset
...
from rodnet.core.post_processing import write_dets_results_single_frame_submit
...
def parse_args():
    ...
    parser.add_argument('--res_dir', type=str, default='./submit_online/', help='directory to save testing results')
    ...
    parser.add_argument('--valid', action="store_true", help='False: valid with GT')
...
if __name__ == "__main__":
    ...
    data_root = dataset_configs['pkl_root']
    seq_names = sorted(os.listdir(os.path.join(data_root, dataset_configs['test']['subdir'])))
    print(seq_names)
    print('length of seqs: %d' % len(seq_names))

    for seq_name in seq_names:
        seq_name = seq_name.split('.')[0]
        f = open(os.path.join(test_res_dir, seq_name+'.txt'), 'w')
        f.close()
    ...
    for subset in seq_names:
        ...
        crdata_test = CRDataset(data_dir=args.data_dir, dataset=dataset, config_dict=config_dict, split='test',
                                noise_channel=args.use_noise_channel, subset=subset.split('.')[0], is_random_chirp=False)

        print("Length of testing data: %d" % len(crdata_test))
        dataloader = DataLoader(crdata_test, batch_size=1, shuffle=False, num_workers=16, collate_fn=cr_collate)
        ...
        for iter, data_dict in enumerate(dataloader):
            ...
            radar_paths = data_dict['radar_paths'][0]
            # print(len(radar_paths))
            ...
            save_path = os.path.join(test_res_dir, seq_name+'.txt')
            ...
            # print(radar_paths[0])
            # print(radar_paths[-1])
            start_frame_name = radar_paths[0].split('/')[-1].split('.')[0].split('_')[0]
            end_frame_name = radar_paths[-1].split('/')[-1].split('.')[0].split('_')[0]
            start_frame_id = int(start_frame_name)
            end_frame_id = int(end_frame_name)

            print("Testing %s: %s-%s" % (seq_name, start_frame_name, end_frame_name))
            tic = time.time()
            # print(data.shape) # [1, 2, 16, 128, 128]
            ...
            # print(test_configs['test_stride'])
    ...
    print("ave time: %f, total_count: %d, tt: %f, g_total_time: %f" % (total_time / total_count, total_count, total_time, global_total_time))
