...
from rodneto.core.post_processing import post_process, post_process_single_frame
from rodneto.core.post_processing import write_dets_results, write_dets_results_single_frame, write_dets_results_single_frame_submit
...
def parse_args():
    ...
    parser.add_argument('--res_dir', type=str, default='./valid_results/', help='directory to save testing results')
    parser.add_argument('--use_noise_channel', action="store_true", help="use noise channel or not")
    parser.add_argument('--valid', action="store_true", help='False: test with GT, True: demo without GT')
...
if __name__ == "__main__":
    ...
    elif model_configs['type'] == 'simple':
        from rodnet.models.backbones.simple import get_pose_net
        from rodnet.models.backbones.config import config
    elif model_configs['type'] == 'CDCMNet':
        from rodneto.models import RODNetCDCMNet as RODNet
    elif model_configs['type'] == 'CDCTDC':
        from rodneto.models import RODNetCDCTDC as RODNet
    elif model_configs['type'] == 'HRNet':
        from rodneto.models import SimplifiedHRNet3d as RODNet
    elif model_configs['type'] == 'HRNet2':
        from rodneto.models import HRNet3d as RODNet
    elif model_configs['type'] == 'HRMNet':
    ...
    configName=model_configs['type']
    if model_configs['type'] == 'CDC' or model_configs['type'] == 'CDC4' or configName=='CDCTDC':
        ...
        rodnet = get_pose_net(cfg=config, is_train=True, num_layers=34).cuda()
    elif model_configs['type'] == 'CDCMNet':
        rodnet = RODNet(n_class_test, n_features=2, n_input_dim=2).cuda()
    elif configName == 'HRNet':
        channels = model_configs['channels']
        rodnet = RODNet(2, n_class_test, channels).cuda()
    elif configName == 'HRNet2' or configName == 'HRMNet':
        channels = model_configs['channels']
        rodnet = RODNet(2, n_class_test, channels).cuda()
    else:
        raise TypeError
    ...
    if args.valid:
        seq_names = sorted(os.listdir(os.path.join(data_root, dataset_configs['valid']['subdir'])))
    else:
    ...
    for subset in seq_names:
        ...
        if args.valid:
            ...
            crdata_test = CRDataset(data_dir=args.data_dir, dataset=dataset, config_dict=config_dict, split='demo',
                                    noise_channel=args.use_noise_channel, subset=subset, is_random_chirp=False)
        print("Length of validation data: %d" % len(crdata_test))
        dataloader = DataLoader(crdata_test, batch_size=1, shuffle=False, num_workers=8, collate_fn=cr_collate)
        ...
        for iter, data_dict in enumerate(dataloader):
            ...
            for i in range(test_configs['test_stride']):
                ...
                write_dets_results_single_frame_submit(res_final, cur_frame_id, save_path, dataset)
                if iter%20==0:
                    confmap_pred_0 = init_genConfmap.confmap
                    res_final_0 = res_final
                    img_path = image_paths[i]
                    if len(data.shape)==6:
                        tmpData=data.numpy()[0,:,i,0,:,:]
                    else:
                        tmpData=data.numpy()[0,:,i,:,:]
                    radar_input = chirp_amp(tmpData, radar_configs['data_type'])
                # else:
